    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      decoratedChild = Container(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            border: widget.controller.hasTwoFingers
                ? Border.all(
                    width: 2,
      ...
      return Stack(
        children: <Widget>[
          Positioned(
            ...
            child: Transform(
              ...
              child: _DraggablePoint(
                ...
                state: widget.controller.hasTwoFingers ? null : widget.controller,
                onlyOneFinger: true,
                onTapDown: () {
                  widget.controller.setSelectedAssetId(hashCode.toString());
                },
                onTapUp: () {},
                ...
                onDrag: (d) {
                  // if (widget.controller.hasTwoFingers) return;
                  //get the id of this sticker
                  setState(() {
                  ...
                  children: [
                    ...
                    if (widget.canTransform && isTouchInputSupported) ...[
                      Positioned(
                        ...
                        child: widget.controller.hasTwoFingers ? Container() : topLeftCorner,
                      ),
                      Positioned(
                        right: (normalizedWidth / 2) - (_floatingActionDiameter / 2) + (_cornerDiameter / 2) + (_floatingActionPadding / 2),
                        child: widget.controller.hasTwoFingers ? Container() : topCenter,
                      ),
                      Positioned(
                        bottom: _floatingActionPadding / 2,
                        left: _floatingActionPadding / 2,
                        child: widget.controller.hasTwoFingers ? Container() : deleteButton,
                      ),
                      Positioned(
                        top: normalizedHeight + _floatingActionPadding / 2,
                        left: normalizedWidth + _floatingActionPadding / 2,
                        child: widget.controller.hasTwoFingers ? Container() : bottomRightCorner,
                      ),
                      Positioned(
                        top: _floatingActionPadding / 2,
                        right: _floatingActionPadding / 2,
                        child: widget.controller.hasTwoFingers ? Container() : rotateAnchor,
                      ),
                    ],
                  ],
                ),
          ...
          if (widget.controller.hasTwoFingers && widget.controller.selectedAssetId == hashCode.toString())
            Container(
              ...
              child: Stack(
                children: [
                  Text(widget.controller.selectedAssetId),
                  Center(
                      child: Column(
                    children: [
                      Text(hashCode.toString()),
                      Text(angle.toString()),
                      Text(widget.controller.fingerCount.toString()),
                    ],
                  )),
                  _DraggablePoint(
                    onSecondaryTap: () {
                    ...
                    onDrag: (d) {
                      if (widget.controller.onDrag(hashCode.toString()) == true) {
                        setState(() {
                          position = Offset(position.dx + d.dx, position.dy + d.dy);
                        });
                      }
...
class _ResizePoint extends StatelessWidget {
  const _ResizePoint({Key? key, required this.onDrag, required this.type, this.onScale, this.iconData}) : super(key: key);
